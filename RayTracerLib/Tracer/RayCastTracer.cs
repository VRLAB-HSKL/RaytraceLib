...
namespace RayTracerLib.Tracer
{
    public class RayCastTracer : AbstractTracer
    {
        ...
        public RayCastTracer(World w) : base(w) 
        {
            //_maxDist = RayTraceUtility.Raycast_Distance;
            //_layerMask = RayTraceUtility.LayerMask;
            //_bgColor = w.BackgroundColor; // RayTraceUtility.GlobalWorld.BackgroundColor;
        }
        ...
        {
            ShadeRec shr = World.HitObjects(ray);
            
            if(shr.HitAnObject)
            {
                shr.SpecRay = ray;
                return shr.Material.Shade(ref shr, depth);
            }
            else
            {
